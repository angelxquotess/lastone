...
def _type(text: str, interval: float = 0.03) -> str:
    ...
    pyautogui.typewrite() SILENTLY DROPS any character it can't map to a
    US-QWERTY key press (e.g. accented letters: à è ì ò ù, £, €, etc.).
    This produced bugs like the input "ci fara' caso" being typed instead
    of "ci farà caso".

    Strategy:
      1) If the text is pure ASCII  -> use typewrite (fast, char-by-char)
      2) Otherwise, if pyperclip is available -> clipboard-paste (reliable
         for every unicode char, including accents, emojis, symbols)
      3) Fallback: try pyautogui.write, and for every character that
         typewrite can't handle we manually copy-and-paste that single
         character so nothing gets silently dropped.
    """
    _require_pyautogui()
    time.sleep(0.3)
    ...
    # --- Non-ASCII path (contains accents / unicode) -----------------
    if _PYPERCLIP:
        try:
            prev_clip = ""
            try:
            ...
            # Restore previous clipboard so we don't leak state
            try:
                if prev_clip:
            ...
            return (f"Typed (clipboard, unicode): "
                    f"{text[:60]}{'…' if len(text) > 60 else ''}")
        except Exception as e:
            print(f"[ComputerControl] ⚠️ clipboard-type failed, falling back: {e}")

    # --- Last-resort fallback: char by char, individually pasting the
    # non-ASCII ones so accented letters are NEVER dropped ------------
    for ch in text:
        if _is_ascii(ch):
            try:
            ...
            if _PYPERCLIP:
                try:
                    ...
                    pass
            # Absolute last resort: try to press the char directly
            try:
                pyautogui.press(ch)
            except Exception:
                pass
    return f"Typed (per-char unicode): {text[:60]}{'…' if len(text) > 60 else ''}"
...
def _smart_type(text: str, clear_first: bool = True) -> str:
    ...
    # Any non-ASCII (accents, unicode) MUST go through clipboard,
    # otherwise pyautogui.typewrite silently drops the character.
    if (len(text) > 20 or not _is_ascii(text)) and _PYPERCLIP:
        pyperclip.copy(text)
        time.sleep(0.1)
        paste_key = "command" if _get_os() == "mac" else "ctrl"
        pyautogui.hotkey(paste_key, "v")
    ...
    # Pure ASCII short text: safe to typewrite
    if _is_ascii(text):
        pyautogui.typewrite(text, interval=0.04)
        return f"Smart-typed: {text[:60]}{'…' if len(text) > 60 else ''}"

    # Non-ASCII but no clipboard: delegate to _type per-char fallback
    return _type(text, interval=0.04)
...
def computer_control(
    ...
    _ACTION_ALIASES = {
        ...
        "enter_text": "type",
        "keyboard_type": "type",
        "smart_typing": "smart_type",
        "smart_write": "smart_type",
        "type_smart": "smart_type",
        "key_press": "press",
        "keypress": "press",
        "press_key": "press",
        "hit_key": "press",
        ...
        "paste_text": "paste",
        "clear": "clear_field",
        "clear_input": "clear_field",
        "erase_field": "clear_field",
        "focus": "focus_window",
        "activate_window": "focus_window",
        "bring_to_front": "focus_window",
        "find_element": "screen_find",
        "click_element": "screen_click",
        "ai_click": "screen_click",
        "fake_data": "random_data",
        "generate_data": "random_data",
        "profile_data": "user_data",
        "memory_data": "user_data",
    }
    action = _ACTION_ALIASES.get(action, action)

    if action in ("select_all", "selectall", "select"):
        select_key = "command" if _get_os() == "mac" else "ctrl"
        return _hotkey(select_key, "a")

    if action == "scroll_up":
        return _scroll("up", int(params.get("amount", 3)))
    if action == "scroll_down":
        return _scroll("down", int(params.get("amount", 3)))
    if action == "scroll_left":
        return _scroll("left", int(params.get("amount", 3)))
    if action == "scroll_right":
        return _scroll("right", int(params.get("amount", 3)))
